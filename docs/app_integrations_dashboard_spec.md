...
- **Assignee changes propagate on sync/webhook**: when a later sync matches the provider-side assignee to an app user **by email**, the task's assignee follows the provider (and the user is added as a project member). When no match is found, the task's current assignee is left untouched — a sync never resets a task back to the importing admin, and never assigns tasks to deactivated users.
...
Mapping notes the UI may surface: issue **status** maps by name first ("In Progress", "Done", "Cancelled"…) with Jira's status *category* as fallback, so custom workflows still land on a sensible app status; **priority** maps Highest/High → high, Medium → medium, Low/Lowest → low; **due date** → task deadline; **subtasks import as ordinary tasks**. Assignee matching tries, in order: (1) the Jira payload **email** (authoritative when Atlassian exposes it), (2) the **connected account's own email** captured at OAuth time (matched by Atlassian accountId) — both are *exact* matches that follow provider-side reassignments; (3) an **exact, unambiguous display-name match** against active company users (Unicode-normalized, case/whitespace-insensitive; skipped when several app users share the name, and skipped when Jira DID expose an email that matches nobody). Name matches are *heuristic*: they only claim tasks still sitting on the importing admin (never overriding a manual in-app reassignment or an email-based match), and they never grant project-manager rights from the Jira project lead. Atlassian hides emails behind user privacy settings (GDPR) — expect `email: null` in `unmatched_jira_users`; truly unmatched tasks fall back to the importing admin on first import and keep their current assignee on later syncs. UI hint for the warning list: the user can make their email visible at id.atlassian.com → Profile and visibility → Contact → Email address → "Anyone", or an admin can align the app user's display name with the Jira display name — the next Sync then re-maps their still-unmapped tasks automatically.
