...
const EditBudgetAndHoursStep = ({
  ...
  const form = useForm<z.infer<typeof addBudgetAndHoursSchema>>({
    resolver: zodResolver(addBudgetAndHoursSchema) as Resolver<
      z.infer<typeof addBudgetAndHoursSchema>
    >,
    defaultValues: {
  ...
  async function onSubmit(values: z.infer<typeof addBudgetAndHoursSchema>) {
    ...
    const finalData = {
      ...
      user_ids: data?.members.map(
        (member: { id: number; name: string }) => member?.id,
      ),
  ...
  return (
    <div>
      <h2 className=" text-xl font-medium mb-4 text-headingTextColor dark:text-darkTextPrimary">
        Edit Budget
      </h2>
        ...
        <Form {...form}>
          ...
          >
            ...
            <div className=" flex items-center justify-between gap-3">
              <button
                onClick={() => setStep(2)}
                className=" bg-primary rounded-lg text-white p-2 cursor-pointer"
                type="button"
              >
                <ChevronLeft size={25} />
              </button>
              {/* <DialogClose asChild> */}
              <button
                className=" bg-primary rounded-lg text-white py-2 px-3 cursor-pointer"
                disabled={loading}
                type="submit"
              >
