 ...
 */
export default function CurrentPlanCard({ plans, activeUserCount }: { plans: IBillingPlan[], activeUserCount:number }) {
  ...
  return (
    <div className="border border-borderColor divide-x divide-gray-100 dark:divide-darkBorder dark:border-darkBorder flex rounded-lg p-3 sm:p-4 bg-white dark:bg-darkPrimaryBg ">
      <div className=" min-w-70 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <p className="text-lg text-subTextColor dark:text-darkTextSecondary mb-1">
            Current plan
          </p>
          <div className="flex items-center gap-2 mb-2">
            ...
            {entitlements.tier && (
              <span className="inline-flex items-center rounded-full bg-primary/10 border border-primary/50 text-primary px-2 py-0.5 text-xs font-medium capitalize">
                {entitlements.tier}
              </span>
            )}
          ...
          <div className="space-y-2 text-sm text-subTextColor dark:text-darkTextSecondary">
            {entitlements.billing_cycle && (
              <p className="flex items-center gap-1.5">
                <BillingIcon size={30} className="bg-[#f3f5fa] dark:bg-[#ececec15] p-1 rounded" />
            ...
            {entitlements.status === "active" &&
              entitlements.current_period_end && (
                <p className="flex items-center gap-1.5">
                  <TrailCalenderIcon size={30} className="bg-[#f3f5fa] dark:bg-[#ececec15] p-1 rounded" />
      ...
      <SeatUsageCard activeUserCount={activeUserCount} />
    </div>
  );
}
