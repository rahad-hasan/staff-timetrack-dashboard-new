...
const EditManualTimeModal = ({
  ...
  const [date, setDate] = useState<Date | undefined>(
    new Date(selectedItem?.start_time),
  );
  ...
  const onSubmit = async (data: z.infer<typeof addManualTimeSchema>) => {
    if (data.date && data.timeFrom && data.timeTo) {
      ...
      const [fromHours, fromMinutes, fromSeconds] = data.timeFrom
        .split(":")
        .map(Number);
      const timeFromISO = new Date(dateOnly);
      timeFromISO.setHours(fromHours, fromMinutes, fromSeconds || 0, 0);

      const [toHours, toMinutes, toSeconds] = data.timeTo
        .split(":")
        .map(Number);
      ...
      setLoading(true);
      try {
        const res = await editManualTimeEntry({
          data: finalData,
          id: selectedItem?.id,
        });
        if (res?.success) {
...
export default EditManualTimeModal;
