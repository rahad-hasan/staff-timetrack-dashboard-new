...
const AllScreenShortsModal = ({
  screenShorts,
  modalOpen,
  setModalOpen,
  selectedImage,
}: {
  screenShorts: IAllScreenshot[];
  modalOpen: any;
  setModalOpen: any;
  selectedImage: IAllScreenshot;
}) => {
  ...
  if (start < 0) start = 0;
  if (start > screenShorts.length - 4)
    start = Math.max(0, screenShorts.length - 4);
    ...
    >
      <div className="absolute top-5 right-5 flex gap-5 z-50">
        <ZoomOut
          ...
          className="text-white cursor-pointer h-8"
        />
        <ZoomIn
          onClick={() => setZoom((z) => Math.min(1.25, z + 0.25))}
          className="text-white cursor-pointer h-8"
        />
        <Fullscreen
      ...
      <div className=" w-full flex justify-center items-center">
        <div className="lg:absolute lg:bottom-4 lg:left-6 min-w-[280px] bg-black/80 backdrop-blur-2xl border border-white/20 rounded-xl text-white shadow-[0_20px_50px_rgba(0,0,0,0.5)] pointer-events-none p-0 overflow-hidden">
          {screenShorts[activeIndex]?.corrupted &&
            screenShorts[activeIndex]?.corrupted !== "NONE" && (
              <div
                className={`flex items-center gap-2 px-4 py-2 border-b border-white/10 ${screenShorts[activeIndex]?.anomaly?.severity === "high" ? "bg-red-500/20 text-red-400" : "bg-yellow-500/20 text-yellow-400"}`}
              >
                <ShieldAlert size={16} className="animate-pulse" />
                <div className="flex flex-col gap-0.5">
                  <span className="text-[10px] font-black uppercase tracking-widest leading-none">
                    {screenShorts[activeIndex]?.anomaly?.type ||
                      screenShorts[activeIndex]?.corrupted}{" "}
                    Detected
                  </span>
          ...
          <div className="p-4">
            <div className="flex justify-between items-start mb-3">
              ...
              <span className="text-[13px] bg-white/10 px-2 py-0.5 rounded border border-white/5">
                {screenShorts[activeIndex]?.format_time
                  ? screenShorts[activeIndex].format_time
                  : "--:--"}
              </span>
            </div>
              ...
              <div>
                <div className="flex justify-between text-[11px] mb-1 uppercase tracking-wider text-white/50 font-semibold">
                  ...
                  <span className="text-white">
                    {screenShorts[activeIndex]?.score}%
                  </span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="flex items-center justify-between bg-white/5 border border-white/5 p-2 rounded-lg">
                  ...
                  <span className="text-xs font-bold">
                    {screenShorts[activeIndex]?.mouse_activity}%
                  </span>
                </div>
                <div className="flex items-center justify-between bg-white/5 border border-white/5 p-2 rounded-lg">
                  <div className="flex items-center gap-2 text-white/60">
                  ...
                  <span className="text-xs font-bold">
                    {screenShorts[activeIndex]?.keyboard_activity}%
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-3 pt-3 border-t border-white/20 flex items-center justify-between opacity-50">
              ...
              <span className="text-[10px] border-white/70">
                Screen: Display 0
                {screenShorts[activeIndex]?.display_name.slice(7)}
              </span>
            </div>
          </div>
        </div>
