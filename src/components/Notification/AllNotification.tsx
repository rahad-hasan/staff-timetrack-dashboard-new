...
const NotificationItem = ({
  ...
  const config = REASON_CONFIG[notification.reason] || {
    label: "General",
    icon: Bell,
  ...
  return (
    <div
      className={`group relative flex items-start gap-4 p-5 mb-4 rounded-xl border border-primary/60 dark:border-darkBorder transition-all duration-200 shadow-sm
            ${
    ...
    >
      <div
        className={`flex-shrink-0 w-12 h-12 rounded-xl flex items-center justify-center ${config.colorClass} border border-white/50 dark:border-darkBorder shadow-sm`}
      >
        <Icon size={22} />
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1 flex-wrap">
          {(notification.reason === "work_report" ||
            notification.reason === "task_overdue") && (
            <AlertTriangle size={16} className="text-yellow-600" />
          )}

          <h3
            className={`text-sm sm:text-[15px] font-bold leading-tight ${!notification.is_read ? "text-headingTextColor dark:text-darkTextPrimary" : "text-subTextColor dark:text-darkTextSecondary"}`}
          >
        ...
        <p className="text-sm text-subTextColor dark:text-darkTextSecondary mb-2 line-clamp-2">
          {getPlainText(notification?.data?.note) ||
            "Click to view details about this notification."}
        </p>
    ...
    <div className="w-full mx-auto min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-headingTextColor dark:text-darkTextPrimary">
              Notifications
            </h1>
            <p className="text-sm text-subTextColor dark:text-darkTextSecondary">
              Stay updated with your tasks and meetings
            </p>
          </div>
        </div>
        <button
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <NotificationSubjectSelection
          canSeeUnusualActivity={canSeeUnusualActivity}
          by_reason={notificationCount?.by_reason}
          total={notificationCount.summary[category]}
        ></NotificationSubjectSelection>
      </div>
      ...
      {notifications.length > 0 ? (
        ...
        <div className="text-center py-24 bg-white dark:bg-darkSecondaryBg rounded-[12px] border border-dashed border-gray-200 dark:border-darkBorder">
          <p className="text-subTextColor dark:text-darkTextSecondary font-medium">
