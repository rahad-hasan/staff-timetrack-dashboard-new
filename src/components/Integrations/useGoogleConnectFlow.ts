...
export const useGoogleConnectFlow = (onConnected?: () => void) => {
    ...
    const start = useCallback(async () => {
        ...
        try {
            ...
            const features = "width=600,height=700,menubar=no,toolbar=no,resizable=yes,scrollbars=yes";
            popupRef.current = window.open(url, "stafftime-google-connect", features);
            ...
            timerRef.current = setInterval(async () => {
                ...
                try {
                    const status: any = await getGoogleConnected();
                ...
                if (popupRef.current?.closed) {
                    ...
                    toast.message("Google sign-in window closed");
                }
            }, POLL_INTERVAL_MS);
        } catch (err: any) {
            setBusy(false);
