...
const SideBar = () => {
  ...
  const roleBasedOthersSidebarItems =
    logInUserData?.role === "admin" ||
    logInUserData?.role === "manager" ||
    logInUserData?.role === "hr"
      ? othersSidebarItems
        ...
        >
          <div
            className={`flex items-center justify-between bg-bgPrimary dark:bg-darkPrimaryBg mx-4 rounded-2xl border border-borderColor dark:border-darkBorder/70 ${
              isCollapsed ? "flex-col py-2 px-2 gap-2" : "flex-row py-2.5 px-3"
          ...
          <div className={clsx(isCollapsed ? "px-5" : "px-4", " mt-6 mb-3")}>
            {!isCollapsed && (
              <h2 className="text-xs uppercase text-subTextColor dark:text-darkTextSecondary mb-3">
                Main menu
              </h2>
            )}
            {roleBasedSidebarItems.map((item) => (
              <div key={item.key}>
                <SidebarItem
                  ...
                  subMenuActive={item.subItems.some(
                    (subItem) => subItem.key === activeSubItem,
                  )}
                ...
                >
                  {item.subItems.length > 0 && (
                    <div
                      className={`${isCollapsed ? "absolute left-24 px-3 w-[180px] bg-white dark:bg-darkSecondaryBg shadow-2xl rounded-2xl" : "block"} py-2.5 flex flex-col gap-1 transition-all duration-300`}
                    >
          ...
          <div
            className={clsx(
              isCollapsed ? "mx-5" : "mx-4",
              " pt-3 border-t border-borderColor dark:border-darkBorder",
            )}
          >
            {!isCollapsed && (
              <h2 className="text-xs uppercase text-subTextColor dark:text-darkTextSecondary mb-2">
                Others
              </h2>
            )}
