...
const Notification = ({ unreadCount, notificationsList }: any) => {
    ...
    const getNotificationIcon = (reason: string) => {
        switch (reason) {
            case 'leave': return <Calendar className="w-4 h-4 text-gray-500 dark:text-white/80" />;
            case 'work_report': return <FileText className="w-4 h-4 text-orange-500" />;
            case 'task': return <RefreshCcw className="w-4 h-4 text-gray-500 dark:text-white/80" />;
