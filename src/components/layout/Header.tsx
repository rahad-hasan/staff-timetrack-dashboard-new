...
const Header = ({ data }: { data: { duration: string } }) => {
  ...
  useEffect(() => {
    const handleIncomingData = (data: any) => {
      ...
      setUnreadCount(data ? data?.meta?.total : 0);
    ...
    <div className=" border-b border-borderColor dark:border-darkBorder py-3 lg:py-3.5 2xl:py-5 px-3 lg:px-5 flex items-center justify-between rounded-t-lg dark:bg-darkPrimaryBg">
      <div>
        ...
        <Button
          ...
          variant={"filter"}
        >
          <CirclePlay className="text-primary size-5.5 sm:size-5.5" />
          <span className="  dark:text-darkTextPrimary">{data?.duration}</span>
        </Button>
      </div>
        ...
        <div className="hidden lg:block ">
          ...
          <Link href={`/download`}>
            <Button
              ...
              variant={"outline"}
            >
              <Download className=" text-primary w-8" />
              Download App
            </Button>
          </Link>
        </div>
